...
def main():
    ...
    nodeCount = 10 #number of nodes - 1
    edgeThickness = 1
    unconnectedNodes = []
    connectedNodes = []
    edgeArray = []
    ...
    for n1 in unconnectedNodes:
        for n2 in unconnectedNodes:
            mainSurface.fill(BLACK)
            if n1 == n2:
                pass
            else:
                currentDistance = (((n1.xPos - n2.xPos) ** 2) + ((n1.yPos - n2.yPos) ** 2)) ** 0.5
                edgeArray.append(Edge(n1,n2))
                if currentDistance < shortestDistance:
                    superShortestEdge = Edge(n1,n2)
                    shortestn1 = n1
                    shortestn2 = n2
    ...
    while len(unconnectedNodes) > 0:
        ...
        for connectedEdge in connectedEdgeArray:
            pygame.draw.line(mainSurface,GREEN,(connectedEdge.xPos1,connectedEdge.yPos1),(connectedEdge.xPos2,connectedEdge.yPos2),edgeThickness)
        for eachnode in unconnectedNodes:
            pygame.draw.circle(mainSurface,WHITE,(eachnode.xPos,eachnode.yPos),5,0)
        for eachnode in connectedNodes:
            pygame.draw.circle(mainSurface,WHITE,(eachnode.xPos,eachnode.yPos),5,0)
